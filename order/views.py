...
def add_to_cart(request,pk):
    ...
    print("ordered item object",order_item)
    print("order item with index",order_item[0])
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            order_item[0].quantity+=1#oder item was tuple that's why we use [0] to make this a list for changes
            order_item[0].save()
