...
#to stop from checking csrf validation
...
def checkout(request):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            form.save()
            form=BillingForm(instance=saved_address)
            messages.success(request,'Your order has been taken')
            return redirect('home')
    order_qs=Order.objects.filter(user=request.user,ordered=False)
    order_items=order_qs[0].orderitems.all()
    order_total=order_qs[0].get_totals
    return render(request,'payment/checkout.html',context={'form':form,'order_item':order_items,'order_total':order_total,'saved_address':saved_address})
...
def complete(request):
    if request.method == 'POST' or request.method=='post':
        ...
        if status == "VALID":
            ...
            return HttpResponseRedirect(reverse('purchased', kwargs={'val_id':val_id,'tran_id':tran_id}))
        elif status == "FAILED":
